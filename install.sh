#!/bin/sh
# slackctl installer for macOS and Linux.
#
...
#   SLACKCTL_VERSION=v1.2.3   pin a version (default: the latest release)
...
REPO="jjuanrivvera/slackctl"
BINARY="slackctl"
VERSION="${SLACKCTL_VERSION:-}"
...
if [ -z "$VERSION" ]; then
  ...
  [ -n "$VERSION" ] || die "could not determine the latest release; set SLACKCTL_VERSION"
fi
