...
func init() {
	register(func(root *cobra.Command) {
		...
		cmd := &cobra.Command{
			...
			Example: `  slackctl doctor
  slackctl doctor --json`,
...
func runDoctor(cmd *cobra.Command) []check {
	...
	checks = append(checks, check{Name: "active workspace", OK: true, Detail: profileName})
	...
	if err != nil {
		...
		checks = append(checks, check{Name: "API reachable + token valid", OK: true, Detail: fmt.Sprintf("%s @ %s", id.User, id.Team)})
	}
