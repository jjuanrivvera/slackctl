...
func init() {
	register(func(root *cobra.Command) {
		configCmd := &cobra.Command{
			Use:   "config",
			Short: "Inspect and edit slackctl configuration",
...
func configViewCmd() *cobra.Command {
	return &cobra.Command{
		...
		Example: `  slackctl config view
  slackctl config view -o json`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			cfg, err := config.Load()
			if err != nil {
				return err
			...
			view := map[string]any{
				...
				"aliases":         cfg.Aliases,
				"token_storage":   "OS keyring (run `slackctl auth status` to verify)",
			}
...
func configSetCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: `  slackctl config set base_url https://api.telegram.org
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if _, ok := cfg.Profile(args[0]); !ok {
				return fmt.Errorf("no such profile %q — create it with `slackctl auth login --workspace %s`", args[0], args[0])
			}
