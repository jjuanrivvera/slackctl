// Package config resolves slackctl configuration with a manual flag > env > file > default
// precedence (no Viper, per the cliwright house pattern). Profiles let one user drive several
// workspaces; the secret tokens never live here — only in the OS keyring (see internal/auth).
package config
...
// Profile holds the non-secret settings for one workspace.
type Profile struct {
	BaseURL    string `yaml:"base_url,omitempty"`
	AuthMethod string `yaml:"auth_method,omitempty"`
	Team       string `yaml:"team,omitempty"`    // workspace name, from auth.test
	TeamID     string `yaml:"team_id,omitempty"` // workspace id, from auth.test
	UserID     string `yaml:"user_id,omitempty"` // the token's user/bot user id
	BotID      string `yaml:"bot_id,omitempty"`  // bot id, when the token is a bot token
}
...
// Dir returns the configuration directory: $XDG_CONFIG_HOME/slackctl if set, else ~/.slackctl-cli.
func Dir() (string, error) {
	if xdg := os.Getenv("XDG_CONFIG_HOME"); xdg != "" {
		return filepath.Join(xdg, "slackctl"), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
		return "", err
	}
	return filepath.Join(home, ".slackctl-cli"), nil
}
...
// ResolveProfileName applies precedence: explicit flag > SLACKCTL_WORKSPACE >
// SLACKCTL_PROFILE (generic alias) > current_profile > default. A profile is one Slack
// workspace, so SLACKCTL_WORKSPACE is the primary env var.
func (c *Config) ResolveProfileName(flag string) string {
	return FirstNonEmpty(flag, os.Getenv("SLACKCTL_WORKSPACE"), os.Getenv("SLACKCTL_PROFILE"), c.CurrentProfile, DefaultProfile)
}

// FirstNonEmpty returns the first non-empty string — the manual precedence helper used
// across slackctl instead of a config framework.
