...
func TestResolveProfileName_Precedence(t *testing.T) {
	...
	t.Setenv("SLACKCTL_PROFILE", "fromenv")
...
func TestDirAndPath_XDG(t *testing.T) {
	...
	dir, err := Dir()
	require.NoError(t, err)
	assert.Equal(t, filepath.Join(base, "slackctl"), dir)
	p, err := Path()
	require.NoError(t, err)
	assert.Equal(t, filepath.Join(base, "slackctl", "config.yaml"), p)
}
...
func TestDir_HomeFallback(t *testing.T) {
	...
	dir, err := Dir()
	require.NoError(t, err)
	assert.Contains(t, dir, ".slackctl-cli")
}
